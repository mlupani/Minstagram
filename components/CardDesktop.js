...
import styles from 'styles/CardDesktop.module.css'
...
const CardDesktop = ({createdAt, userName, img, content, id, avatar, userID, likesUser, comments, place, savesUser, actualUserID, filter }) => {
						...
						<div className={`${styles.containerImg}`}>
							{img.length > 1 ? (
								...
								>
									{img.map((img) => (
										<div
											className={`filter-${img.filterApplied.toLowerCase()}`}
											key={img.img}
										>
											<img src={img.img} />
										</div>
								...
								<figure
									...
									{...favDoubleTap}
									className={`filter-${img[0].filterApplied.toLowerCase()}`}
								>
									<img
										width="100%"
										height="auto"
							...
							<a style={{ textDecoration: "none", color: "white" }}>
								<div
									className={`container ${
										stateFav ? styles.alertShown : styles.alertHidden
									}`}
								>
									<FavFill_icon width="50" height="50" />
								</div>
							</a>
						</div>
					</div>
				</div>
				<div className={styles.flexColComments}>
					<div
						className={`${styles.card}`}
						style={{ paddingLeft: "0px", paddingRight: "0px" }}
					>
										...
										<div className="row">
											<div className="col-12">
												...
												>
													...
													{userID != actualUserID &&
														user?.followsCount?.includes(userID) && (
															<li
																onClick={() => handleCancelFollow(userID)}
																style={{ color: "red", fontWeight: "bold" }}
																className="list-group-item"
															>
																Dejar de seguir
						...
						<div
							className="card-body"
							style={{ fontSize: "13px", paddingRight: "1.5rem !important", 'minHeight': '355px' }}
						>
							{comments &&
								comments.map((comment) => {
										return (
											<Comment
												...
												likeComment={
													postLikesComments
														? postLikesComments.filter(
																(like) => like.idComment == comment.id
													)
														: ""
												}
											/>
						...
						<div
							...
							style={{ fontSize: "13px", paddingRight: "1.5rem !important" }}
						>
							<div className="row">
								<div className="col-10">
									{IDfav ? (
										<a
											onClick={(e) => handleLikeCount(e, "Remove")}
											style={{
												textDecoration: "none",
												color: "#dc3545",
												cursor: "pointer",
										...
										<a
											onClick={(e) => handleLikeCount(e, "Add")}
											style={{
												textDecoration: "none",
												color: "black",
												cursor: "pointer",
								...
								{IDSave ? (
									...
									<div
										onClick={() => handleBookmark(id, "add", img, filter)}
										className="col-2"
										style={{ textAlign: "right" }}
									>
										<Bookmark_icon />
							...
							<p style={{ marginBottom: "10px" }} className="card-text">
								...
								<span>
									{content.length > 90 && !viewDesc ? (
										<>
											{content.substring(1, 90)}{" "}
											<a onClick={() => setViewDesc(true)}>...Ver Mas</a>
										</>
									) : (
										content
									)}
							...
							<Link href="/status/[id]" as={`/status/${id}`}>
								<a style={{ textDecoration: "none"}} className="card-text">
									<small className="text-muted">{timeAgo}</small>
								</a>
							</Link>
							<div
								className={`row ${styles.flexComment}`}
								...
								<div className="col-2">
									...
									>
										<button
											...
											onClick={() =>
												addComments(user, postAct, comentario, postAct)
											}
											disabled={
												!comentario || stateComment == 1 ? "disabled" : ""
											}
