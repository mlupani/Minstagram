...
const FormRegisterGoogle = ({registerWithGoogle, setRegisterWithGoogle, setIsLoading}) => {
    ...
    useEffect(() => {
        ...
        let email = ''
        if(registerWithGoogle.user.providerData[0].email)
            email = registerWithGoogle.user.providerData[0].email
        else
            email = registerWithGoogle.additionalUserInfo.profile.email

        setEmail(email)
        setName(registerWithGoogle.user.displayName)
        setUid(registerWithGoogle.user.uid)
        setPicture(registerWithGoogle.user.photoURL)
    },[])
